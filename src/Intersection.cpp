...
{
    std::lock_guard<std::mutex> lck(vehicle_mutex);
    return _vehicles.size();
}

void WaitingVehicles::pushBack(std::shared_ptr<Vehicle> vehicle, std::promise<void> &&promise)
{
    std::unique_lock<std::mutex> vehiclesLock(vehicle_mutex);
    _vehicles.push_back(vehicle);
    vehiclesLock.unlock();

    std::lock_guard<std::mutex> promisesLock(promise_mutex);
    _promises.push_back(std::move(promise));
}

void WaitingVehicles::permitEntryToFirstInQueue()
{
    std::lock_guard<std::mutex> promisesLock(promise_mutex, std::adopt_lock);
    auto firstPromise = _promises.begin();
    std::lock_guard<std::mutex> vehiclesLock(vehicle_mutex, std::adopt_lock);
